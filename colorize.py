    ...
    for label in labels:
        ...
        if max_hor - min_hor > 5 and max_ver - min_ver > 5:
            if refine(img):
                ...
                color = color_map[cluster.predict(fea)]
                #pdb.set_trace()
...
def extract_strokes(img_name):
    """Extract strokes from a given image."""
    if img_name[-4:] == ".png":
        img = cv2.imread("/home/sanny/honours/50data/" + img_name, 0)
        ...
        #folder = "/home/sanny/data/strokes/"
        #cv2.imwrite(folder + img_name + ".tiff", clr_img[i])
   


bank = pickle.load(open("banks/Py2.7/J34_3.pkl", "rb"))
data = "/home/sanny/honours/50data/"
