#----------------------------------------------------------------
Gabor_path = "data_fullimg_csv/Gabor_fullimg/"
GSCM_path = "data_fullimg_csv/GSCM_fullimg/"
train_file = open("data_fullimg_csv/Gabor_GSCM.csv","r")
train_data = []
train_class = []
...
#-----------------------------------------------------------------
    ...
    for data in train_data[i][32:92]:
        GSCM_all.write(str(data)+',')
    GSCM_all.write(label+'\n')

    #GSCM_1
    for data in train_data[i][32:44]:
        GSCM_1.write(str(data)+',')
    GSCM_1.write(label+'\n')

    #GSCM_2
    for data in train_data[i][44:56]:
        GSCM_2.write(str(data)+',')
    GSCM_2.write(label+'\n')

    #GSCM_3
    for data in train_data[i][56:68]:
        GSCM_3.write(str(data)+',')
    GSCM_3.write(label+'\n')

    #GSCM_4
    for data in train_data[i][68:80]:
        GSCM_4.write(str(data)+',')
    GSCM_4.write(label+'\n')

    #GSCM_5
    for data in train_data[i][80:92]:
