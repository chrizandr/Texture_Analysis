"""Fixed size block construction with complete components."""
...
from block import get_connected_components, refine


def find_position(flag, shape):
    """Find the position to place the next component."""
...
def get_text_blocks(components):
    """Create blocks from the components."""
    blocks = list()
...
def extract(name):
    ...
    if name[-4:] == ".tif":          # Make sure that only appropriate files are processed [add 'or' conditions for other filetypes]
        ...
        for block in blocks:
            assert cv2.imwrite("/home/chris/honours/bangla_blocks/" + name[0:-4] + str(count) + ".tif", block)
